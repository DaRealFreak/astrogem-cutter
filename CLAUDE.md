...
- **Confirmation gate** (`--confirm-risk F`, `--confirm-min-coeff N`): Interactive pause for high-stakes decisions in `auto`. Setting either flag activates the gate (unset flag defaults to 0.0 / 0). Three conceptual gates, all additionally gated by `--confirm-min-coeff` (side-coefficient floor): (1) **finish-vs-continue** — goal is met and continuing has side-coefficient upside; pauses if `P(losing the goal if you keep cutting) >= --confirm-risk`; (2) **off-goal keep** — goal is structurally infeasible or no feasible offer exists, but the gem's side coefficient is valuable; (3) **reset** — auto would spend the reset ticket on a valuable gem. The risk metric is `1 - P(goal still met at run end if you keep cutting)`, computed from a reroll-aware DP table built with `early_finish=False`. In `auto`, a gated turn prints a `[CONFIRM]` panel and blocks on a global F1-F4 hotkey menu (`confirm_choices` lists the selectable actions; Escape aborts the run); the chosen action replaces auto's recommendation. The simulator runs non-interactively — it executes the recommended action but records a `confirm` marker in the turn log on gated turns so behavior is auditable across `sim` batches. `--confirm-risk` overrides `--early-finish-coeff` (same finish decision, different math).

...
- **Side-value finish** (`--endgame-risk F`): `_side_value_finish_decision` consults the `SideValueTable` DP at every goal-met turn to decide finish-vs-continue. `gem_value = side_coeff + tier_bonus(total_points)`, where `tier_bonus` adds `--relic-coeff` for ≥16 points and `--ancient-coeff` for ≥19 points. The decision never finishes while a free reroll remains (rerolls are free; gem supply is the bottleneck). Once rerolls are exhausted it finishes when `finish_val >= process_ev + --endgame-risk` (default 0.0). Gate-on (`--confirm-min-coeff N`) surfaces a side-value FINISH as an F1-F4 prompt for gems above the coefficient floor.
- **Relic+ tracking** (`--relic-coeff N`, `--relic-reroll-threshold F`): A separate DP table computes P(total_points >= 16) from the current state each turn. `--relic-coeff N` is the coefficient-equivalent worth of reaching relic+ grade (≥16 points), added to `gem_value` in the side-value DP; default 0. `--relic-no-early-finish` is retired — its role is folded into `--relic-coeff`. `--relic-reroll-threshold F` re-enables the extra reroll ticket mid-run when P(relic+) from the current state crosses the threshold, overriding `--reroll-min-coeff` gating; default 0.0 (disabled). P(relic+) is displayed in `sim` turn logs, `live` analysis, `auto` turn headers, and `stats` output.
- **Ancient tracking** (`--ancient-coeff N`): `--ancient-coeff N` is the coefficient-equivalent worth of reaching ancient grade (≥19 points), added to `gem_value` in the side-value DP on top of any relic+ bonus; default 0.
- **Confirmation gate** (`--confirm-min-coeff N`): Interactive pause for high-stakes decisions in `auto`. `--confirm-min-coeff N` is the sole confirm-gate knob — it activates the gate and prompts on every side-value FINISH for a gem whose side coefficient is above the floor. Two additional gates, also gated by `--confirm-min-coeff`: (2) **off-goal keep** — goal is structurally infeasible or no feasible offer exists, but the gem's side coefficient is valuable; (3) **reset** — auto would spend the reset ticket on a valuable gem. In `auto`, a gated turn prints a `[CONFIRM]` panel and blocks on a global F1-F4 hotkey menu (`confirm_choices` lists the selectable actions; Escape aborts the run); the chosen action replaces auto's recommendation. The simulator runs non-interactively — it executes the recommended action but records a `confirm` marker in the turn log on gated turns so behavior is auditable across `sim` batches. `--confirm-risk` is retired.

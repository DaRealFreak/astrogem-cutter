...
- **Decision** at a goal-met turn: compute `finish_val = gem_value(current)` and `continue_val = max(process EV, reroll value)`, where `process EV` averages `V` over the 4 *actual* visible offers (mirroring how `GoalProbabilityTable.expected_prob_after_click` values a click) and `reroll value` is `V[state, turns_left, rerolls - 1]`. **Finish iff `finish_val >= continue_val`** (plus the `--endgame-risk` margin — see Integration sketch). The table's own `V` includes the finish-now option, so it is the destination-value oracle for those two terms, not a value compared directly.
...
- **Gate off** (`--confirm-min-coeff` unset): finish iff `finish_val >= continue_val + endgame_risk`. `--endgame-risk N` (float, default `0`) is the unattended risk margin — `0` finishes as soon as continuing cannot beat stopping (EV-optimal); a large `N` finishes only when stopping wins by `N`, i.e. plays to the last turn. A free reroll is still preferred to finishing, and mid-run with no reroll the decision defers (a single bad offer set must not end a goal-met multi-turn gem) — unchanged from today.
